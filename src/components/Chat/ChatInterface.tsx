...
import { VoiceButton } from "../voiceRecognisation";
...
const toDDMMYYYY = (y: number, m: number, d: number) =>
  `${pad(d)}-${pad(m)}-${y}`;
...
// Normalize data:image inline tokens so they render outside tables
function normalizeDataImageTokens(md: string): {
  cleaned: string;
  imgs: string[];
} {
  if (!md) return { cleaned: md, imgs: [] };
  md = md.replace(
...
interface ChatInterfaceProps {
  ...
  refetchSessions: (sessionId: any) => void;
}
...
type PaginatedResp = {
  messages: ChatDetail[]; // oldest -> newest within page
  nextCursor: number | string | null; // cursor for OLDER page
  hasMore: boolean;
};
...
// util
const getTempId = () => Math.floor(Math.random() * -1_000_000);
const getLastId = (arr: ChatDetail[]) =>
  arr.length ? arr[arr.length - 1].id : null;
const appendOnlyNew = (prev: ChatDetail[], incoming: ChatDetail[]) => {
...
export function ChatInterface({
  ...
  async function copyRenderedHTML(messageId: number, key: string) {
    ...
    if (!root) return toast.error("Nothing to copy");
  ...
  // Table copy helpers (TSV + HTML)
  function tableToTSV(tableEl: HTMLTableElement): string {
  ...
  async function copyTable(tableEl: HTMLTableElement) {
    try {
      if ("ClipboardItem" in window) {
        const html = tableEl.outerHTML;
        const text = tableToTSV(tableEl);
        const item = new (window as any).ClipboardItem({
          "text/html": new Blob([html], { type: "text/html" }),
          "text/plain": new Blob([text], { type: "text/plain" }),
        });
  ...
  function handleCopyTableClick(
    ...
    if (!table) {
      toast.error("No table found to copy");
      return;
    }
    copyTable(table);
  }
  ...
  function scrollToBottom(smooth = true) {
    ...
    // Prefer sentinel for iOS/Android reliability
    if (bottomSentinelRef.current?.scrollIntoView) {
      try {
  ...
  function scrollToBottomWithKeyboardSafety() {
    scrollToBottom(true);
    console.log("scrollToBottomWithKeyboardSafety called");
    // After virtual keyboard/layout settles
    setTimeout(() => scrollToBottom(false), 250);
  }

  // Re-stick on size changes (images, fonts, late content)
  useEffect(() => {
    const el = containerRef.current;
  ...
  const scrollToBottomRAF = () =>
    requestAnimationFrame(() => scrollToBottomWithKeyboardSafety());

  // Extract server messages from askQuestionFn response (adapt as per your API)
  function extractServerMessagesFromSend(resp: any): ChatDetail[] | null {
    const m1 = resp?.data?.message;
    const m2 = resp?.data?.messages;
    const m3 = resp?.message;
    if (Array.isArray(m2)) return m2 as ChatDetail[];
    if (m1) return [m1 as ChatDetail];
    if (m3 && typeof m3 === "object") return [m3 as ChatDetail];
    return null;
  }

  // Replace an optimistic message by temp id
  function replaceOptimistic(tempId: number, real: ChatDetail[]) {
    snapshotAutoStick();
    setMessages((prev) => {
      const withoutTemp = prev.filter((m) => m.id !== tempId);
      return appendOnlyNew(withoutTemp, real);
    });
    scrollToBottomRAF();
  }

  // Lightweight "newer" fetch (append-only, no full refetch)
  const fetchNewer = useCallback(async (sid: string | number) => {
    try {
      const resp = await chatDetailFn(Number(sid), null, PAGE_LIMIT);
      const { messages: latest } = normalizeResp(resp);
      if (!latest?.length) return;
      snapshotAutoStick();
      setMessages((prev) => appendOnlyNew(prev, latest));
      scrollToBottomRAF();
    } catch (e) {
      console.error("fetchNewer error", e);
    }
  }, []);
  ...
  const { mutateAsync: sendMessage, isPending: sending } = useMutation({
    ...
    onSuccess: async (data, variables) => {
      if (!sessionId) {
        const newChatId = (data as any)?.data?.chatId;
        if (newChatId) refetchSessions(newChatId);
      }
      // Drop any stale optimistic placeholders
      setMessages((prev) =>
        prev.filter((m) => m.id >= 0 || m.aiAnswer !== "-1")
      );

      const serverMsgs = extractServerMessagesFromSend(data);
      if (serverMsgs && serverMsgs.length > 0 && variables.clientMessageId) {
        replaceOptimistic(variables.clientMessageId, serverMsgs);
      } else if (sessionId) {
        await fetchNewer(sessionId);
      }
  ...
  const handleSend = useCallback(async () => {
    ...
    const optimistic: ChatDetail = {
      id: tempId,
      sql_code: null,
      aiAnswer: "-1",
      categoryTag: category || null,
      chatId: sessionId ? Number(sessionId) : -1,
  ...
  }, [input, user, sessionId, category, sendMessage]);
  ...
  function normalizeResp(resp: any): PaginatedResp {
    if (!resp) return { messages: [], nextCursor: null, hasMore: false };
  ...
  // Initial fetch (latest page)
  const fetchInitial = useCallback(
    async (sid?: string | null, jumpToBottom = false) => {
      if (!sid) {
        setMessages([]);
        setNextCursor(null);
        setHasMore(false);
  ...
  // Fetch older (cursor) with scroll restore
  const fetchOlder = useCallback(async () => {
    ...
    try {
      ...
      setMessages((prev) => {
        const seen = new Set(prev.map((m) => m.id));
        const deduped = olderMsgs.filter((m) => !seen.has(m.id));
        return [...deduped, ...prev];
      });
  ...
  // On session change: reset and load latest
  useEffect(() => {
  ...
  // Auto-stick to bottom on new items
  // useEffect(() => {
  //   if (shouldStickToBottomRef.current) scrollToBottomWithKeyboardSafety();
  // }, [messages.length]);
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (first?.isIntersecting) fetchOlder();
      },
      { root: containerRef.current || null, rootMargin: "10px", threshold: 0.1 }
    );
    io.observe(el);
  ...
  /* =========================
     Memoized Markdown bubble
     ========================= */
  const MarkdownBubble = useMemo(
    () =>
      React.memo(function MarkdownBubble({
        d,
        copiedKey,
        copyRenderedHTML,
        renderedRefs,
        ...
        copyRenderedHTML: (id: number, key: string) => void;
        renderedRefs: React.MutableRefObject<
          Record<number, HTMLDivElement | null>
        >;
      }) {
        const formattedDate =
          d?.aiAnswer !== "-1" ? formatDatesInMarkdown(d?.aiAnswer) : "";
        ...
        return (
          <div className="flex justify-start">
            ...
            >
              ...
              <div
                className="max-w-full relative overflow-x-hidden"
                data-copy="md"
              >
                <article className="prose relative prose-slate text-sm p-2 lg:p-2  max-w-none md-scroll">
                  <ReactMarkdown
                    remarkPlugins={[remarkGfm]}
                    rehypePlugins={[rehypeRaw, rehypeHighlight]}
                    components={{
                      table: (props) => (
                        <div className="md-table-wrap group ">
                          <div className="absolute right-0 m-0.5 top-0 hidden gap-2 group-hover:flex z-10">
                            <button
                              type="button"
                              onClick={(e) => handleCopyTableClick(e)}
                              className="text-[11px] flex gap-1 px-2 py-1 rounded-lg border border-slate-300 bg-white/90 hover:bg-white shadow-sm"
                              title="Copy table "
                            >
                              <Copy className="w-3.5 h-3.5 mt-0.5" /> Table
                            </button>
                          </div>
                      ...
                      p: (props) => <p className="md-table-p" {...props} />,
                      code({
                        inline,
                        children,
                        ...props
  ...
  return (
    // Use dynamic viewport to avoid mobile address bar bug.
    <div className="h-[100dvh] flex flex-col bg-white relative w-full max-w-[100vw] overflow-x-hidden">
      {/* Header */}
      ...
      >
              ...
              return (
                <div key={d.id} className="space-y-3 ">
                  ...
                  <div className="flex justify-end my-6">
                    <div className="relative max-w-3xl min-w-0 rounded-2xl px-2.5 lg:px-6 py-2.5 lg:py-4 pb-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white">
                      <button
                        onClick={() =>
                      ...
                      <p className="whitespace-pre-wrap leading-relaxed break-words text-xs md:text-sm">
                        {d.question}
                      </p>
                  ...
                  {d.aiAnswer !== "-1" ? (
                    <MarkdownBubble
                      d={d}
                      copiedKey={copiedKey}
                      copyRenderedHTML={copyRenderedHTML}
        ...
        {/* Bottom sentinel (always the last thing inside the scrollable container) */}
        <div ref={bottomSentinelRef} style={{ height: 1 }} />
      </div>

      {/* Spacer to avoid composer overlap */}
      <div className="h-24 lg:h-20"></div>

      {/* Composer */}
      <div className="fixed md:absolute bottom-0 w-full">
        <div className="border-t border-slate-200 bg-white p-1.5 md:p-2 sticky bottom-0 z-10 [padding-bottom:env(safe-area-inset-bottom)]">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-end gap-2 z-50 pb-1 lg:mb-0 md:gap-3">
              <textarea
                ref={textAreaRef}
                value={input}
...
//         // Also refetch initial messages to sync the messages state
//         await fetchInitial(sessionId);
//       }
//       if (sessionId === null) {
...
//   // Sync details to messages when details change (for new messages)
//   useEffect(() => {
//     if (details.length > 0 && sessionId) {
//       const normalized = normalizeResp(chatDetailsResp);
...
//       }
//     }
//   }, [details, sessionId, chatDetailsResp]);
...
//                                 th: (props) => (
//                                   <th
//                                     className="md-th !text-center"
//                                     {...props}
//                                   />
//                                 ),
//                                 td: (props) => (
//                                   <td
//                                     className="md-td !text-center"
//                                     {...props}
//                                   />
//                                 ),
//                                 a: (props) => (
